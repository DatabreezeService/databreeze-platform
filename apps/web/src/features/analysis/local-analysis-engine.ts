...
export function executeLocalAnalysis(
  ...
  const targetDataset = datasetId
    ? localDataStore.getDataset(datasetId)
    : datasets[0];
  ...
  const datasetName = targetDataset?.label ?? (locale === 'vi-VN' ? 'Dữ liệu không gian làm việc' : 'Workspace data');

  if (!tabular || tabular.rows.length === 0) {
    return {
      answerText: locale === 'vi-VN'
  ...
  if (measureCol && dimCol) {
    ...
    const topPct = totalMeasure > 0 && topEntry ? ((topEntry[1] / totalMeasure) * 100).toFixed(1) : '0';
    ...
    if (locale === 'vi-VN') {
      answerLines.push(`Dựa trên dữ liệu **${datasetName}** (${tabular.totalRows.toLocaleString('vi-VN')} dòng):`);
      answerLines.push('');
      if (topEntry) {
        answerLines.push(`- **${topEntry[0]}** dẫn đầu với **${formatCurrencyOrNumber(topEntry[1], isCurrency)}** (chiếm ${topPct}% tổng số).`);
      }
      ...
      answerLines.push('Tôi đã tạo biểu đồ tương ứng bên dưới. Bạn có thể nhấn **➕ Thêm vào Bảng điều khiển** để ghim thẻ này lên Canvas.');
    } else {
      answerLines.push(`Based on data from **${datasetName}** (${tabular.totalRows} rows):`);
      answerLines.push('');
      if (topEntry) {
        answerLines.push(`- **${topEntry[0]}** leads with **${formatCurrencyOrNumber(topEntry[1], isCurrency)}** (${topPct}% of total).`);
      }
      for (const [lbl, val] of entries.slice(1, 4)) {
        answerLines.push(`- **${lbl}**: ${formatCurrencyOrNumber(val, isCurrency)}`);
      }
      ...
      answerLines.push('I created a chart proposal below. You can click **➕ Add to Dashboard** to pin it to Canvas.');
    }
    ...
    return {
      ...
      chartProposal: {
        ...
        type: chartType,
        title: chartTitle,
        summary: locale === 'vi-VN' ? `Phân bổ ${measureCol.name} theo ${dimCol.name}` : `${measureCol.name} distribution by ${dimCol.name}`,
  ...
  return {
    ...
    chartProposal: {
      ...
      summary: locale === 'vi-VN' ? `Tổng bản ghi trong ${datasetName}` : `Total records in ${datasetName}`,
