...
**Secret name:** `databreeze/{staging|production}/openai/receipt-ocr`  
...
3. Create a service credential in the OpenAI console and paste it **only** into AWS Secrets Manager secret `databreeze/<env>/openai/receipt-ocr`.
...
aws secretsmanager describe-secret --secret-id databreeze/staging/openai/receipt-ocr --region ap-southeast-1 --query ARN
```
