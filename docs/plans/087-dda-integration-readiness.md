...
**Goal:** Integrate the independent DDA lanes without weakening contracts, prove deterministic Local/Cloud equivalence, reproduce the complete cross-platform journey, and distinguish fixture/partial evidence from production verification.
...
- A fixture-only UI, fake OCR adapter, in-memory repository, skipped real-device/emulator check, unevaluated OpenAI model, or unmeasured performance target is recorded as partial, never verified production evidence.
...
**Merge order:** 081, 082, 084, 083, 085, 086. The refresh lane precedes canvas publication so the viewer integrates against final snapshot/freshness APIs. The complete program includes Desktop, Android, and the live server-side OpenAI adapter; a blocked lane or fake provider remains partial and cannot satisfy production scope.
...
### Task 3: Run the golden cross-platform journey
...
The runbook includes prerequisites, one-command reset/verification, expected IDs/hashes/counts, Vietnamese default screens, English switch, real provider/device configuration, timings, known partial components, and exact production blockers. A fallback run may aid diagnosis but cannot replace real-device and live-provider release evidence.
...
- Update: `docs/evidence/dda/release-evidence.md`
...
1. For every DDA record, attach only existing code/test/release evidence and passing commands. Use `partial` for fixture-only or incomplete provider/platform/security/performance evidence.
...
4. List remaining plan 400 gates, owners, dependencies, and rollback/release strategy. Do not claim production completion from the golden journey alone.
...
- The release evidence distinguishes real services, deterministic fakes, in-memory adapters, skipped hardware/provider evidence, and production gaps.
