...
| JRA-033 | P0 | Every executable typed Job shall carry one immutable server-authored `ExecutionWorkloadEnvelope` bound to the exact action/version/handler digest, input ArtifactVersion handles (ID, schema, byte length, SHA-256), bounded server-derived parameters, output policy, deadline, locale, TenantScope, attempt and canonical hash. Admission shall create it transactionally with the Job and any BUA reservation; claim and execution shall re-authorize the envelope and current lease before any input transfer. Workers shall receive only the opaque envelope reference/hash and authenticated resolver responses, never caller-supplied parameters, storage paths, URLs, credentials, or workspace enumeration authority. |
