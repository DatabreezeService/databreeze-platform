...
export function createIamTenantContextV1(input: {
  ...
  readonly workspaceAuthorizationEpoch?: unknown;
  ...
  if (
    input.workspaceAuthorizationEpoch !== undefined &&
    (typeof input.workspaceAuthorizationEpoch !== 'number' ||
      !Number.isSafeInteger(input.workspaceAuthorizationEpoch) ||
      input.workspaceAuthorizationEpoch < 1)
  )
    return rejected('INVALID_EPOCH');
