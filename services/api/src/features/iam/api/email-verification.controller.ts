...
import { ApiOkResponse, ApiOperation, ApiServiceUnavailableResponse, ApiTags, ApiUnauthorizedResponse } from '@nestjs/swagger';
import type { FastifyReply } from 'fastify';

import { EmailVerificationService, IAM_EMAIL_VERIFICATION_SERVICE } from '../application/email-verification.service.js';
...
import { CSRF_COOKIE_NAME_V1, REFRESH_COOKIE_NAME_V1, REFRESH_COOKIE_PATH_V1, serializeCookieV1 } from './session-cookies.js';

@ApiTags('auth')
@Controller('v1/auth/email-verification')
export class EmailVerificationController {
  public constructor(
    @Optional() @Inject(IAM_EMAIL_VERIFICATION_SERVICE) private readonly service?: EmailVerificationService,
  ) {}

  @Post('verify')
  @HttpCode(200)
  @ApiOperation({ summary: 'Verify registration OTP and atomically activate the personal workspace session' })
  @ApiOkResponse({ type: AuthSessionDto })
  @ApiUnauthorizedResponse({ description: 'The verification command was rejected.' })
  @ApiServiceUnavailableResponse({ description: 'Verification persistence or protected delivery is unavailable.' })
  async verify(
    ...
    if (!result.accepted)
      throw new RegistrationProblemError(result.code === 'VERIFICATION_UNAVAILABLE' ? 'REGISTRATION_UNAVAILABLE' : 'REGISTRATION_REQUEST_REJECTED');
    if (input.clientPlatform === 'web') {
      const csrfToken = randomBytes(32).toString('base64url');
      reply.header('Set-Cookie', [
        serializeCookieV1(REFRESH_COOKIE_NAME_V1, result.value.session.refreshToken, { httpOnly: true, maxAgeSeconds: 2_592_000, path: REFRESH_COOKIE_PATH_V1 }),
        serializeCookieV1(CSRF_COOKIE_NAME_V1, csrfToken, { httpOnly: false, maxAgeSeconds: 2_592_000 }),
      ]);
    }
    return {
      schemaVersion: 4,
