...
export class AgentGrantService {
  ...
  private async requireOwner(context: IamTenantContextV1): Promise<'ALLOWED' | 'DENIED' | 'UNAVAILABLE'> {
    if (!requireWorkspace(context)) return 'DENIED';
    try {
      const membership = await this.memberships.findMembership(context, context.actorId);
      return membership && roleHasPermissionV1(membership.roleId, PERMISSIONS_V1.WORKSPACE_SETTINGS_MANAGE)
        ? 'ALLOWED'
        : 'DENIED';
    } catch {
      return 'UNAVAILABLE';
  ...
  ): Promise<AgentGrantApplicationResultV1<WorkspaceAgentGrantRecordV1 | { readonly level: AgentGrantLevelV1; readonly revision: 0 }>> {
  ...
  public async setMemberGrant(
    ...
    try {
      return await this.grants.withTransaction(context, async (transaction) => {
        ...
        if (existing) {
          if (input.expectedRevision !== existing.revision) throw new Error('IAM_REVISION_CONFLICT');
  ...
  > {
    ...
    const denied: StableIdentifierV1[] = [];
    for (const candidate of input.deniedDatasetIds) {
      const parsed = parseId(candidate);
      if (!parsed.accepted) return rejected(parsed.code);
      denied.push(parsed.value);
    }
    const member = await this.resolveMember(context, memberId.value);
    if (!member.accepted) return rejected(member.code);
    const updatedAt = this.clock().toISOString();
    try {
      return await this.grants.withTransaction(context, async (transaction) => {
        ...
        const record = Object.freeze({
          memberId: member.membershipId,
          deniedDatasetIds: Object.freeze([...denied]),
          revision: nextRevision,
          updatedAt: updatedAt as WorkspaceAgentGrantRecordV1['updatedAt'],
        });
        await transaction.saveDatasetRestrictions(context, record, input.expectedRevision);
        await transaction.bumpAuthorizationEpoch(context);
        return accepted({
          memberId: member.membershipId,
          deniedDatasetIds: record.deniedDatasetIds,
          revision: nextRevision,
        });
      });
    } catch (error) {
  ...
  public async authorize(input: {
    ...
    try {
      ...
      if (restrictions) {
        const denied = new Set(restrictions.deniedDatasetIds);
      ...
      const storedLevel =
        stored?.level ?? defaultAgentGrantLevelForPresetV1(member.preset);
