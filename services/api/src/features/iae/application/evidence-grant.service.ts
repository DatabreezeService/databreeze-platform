...
export type EvidenceGrantServiceErrorV1 = 'GRANT_NOT_FOUND' | 'GRANT_REVOKED' | 'GRANT_EXPIRED' | 'DEVICE_MISMATCH' | 'EPOCH_MISMATCH' | 'EVIDENCE_NOT_FOUND' | 'ARTIFACT_REPOSITORY_UNAVAILABLE';
...
export class EvidenceGrantService {
  ...
  public async issue(context: IamTenantContextV1, input: Omit<Parameters<typeof createEvidenceAccessGrantV1>[0], 'tenantScope'>): Promise<EvidenceGrantServiceResultV1<EvidenceAccessGrantV1>> {
    if (input.authorizationEpoch !== context.authorizationEpoch) return { accepted: false, code: 'EPOCH_MISMATCH' };
  ...
  public async issueForEvidence(context: IamTenantContextV1, input: {
    ...
    if (!this.artifactRepository) return { accepted: false, code: 'ARTIFACT_REPOSITORY_UNAVAILABLE' };
    const versionId = parseStableIdentifierV1(input.versionId);
    const evidenceId = parseStableIdentifierV1(input.evidenceId);
    if (!versionId.accepted || !evidenceId.accepted) return { accepted: false, code: 'INVALID_IDENTIFIER' };
    const source = await this.artifactRepository.withTransaction(context, async (transaction) => {
      ...
      const evidence = (await transaction.listEvidence(context, versionId.value)).find((candidate) => candidate.evidenceId === evidenceId.value);
      return evidence ? { dataMode: version.dataMode, sourceState: evidence.sourceState } : undefined;
    });
