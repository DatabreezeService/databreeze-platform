import { Body, Controller, Get, HttpCode, HttpException, HttpStatus, Inject, Optional, Param, Post, Query, Req } from '@nestjs/common';
import { ApiBearerAuth, ApiBody, ApiOkResponse, ApiOperation, ApiTags } from '@nestjs/swagger';
import { parseStableIdentifierV1 } from '@databreeze/domain/tenant-scope/v1';
import { ApprovalService } from '../application/approval.service.js';
import { APPROVAL_REPOSITORY_PORT, type ApprovalRepositoryPortV1 } from '../application/approval-repository.port.js';
import { REQUEST_TENANT_CONTEXT, type RequestTenantContextPortV1 } from '../../../platform/http/request-tenant-context.port.js';
import { CreateApprovalPolicyDto, CreateApprovalRequestDto, DecideApprovalDto } from './approval.dto.js';
...
function fail(code: string, forcedStatus?: HttpStatus): never {
  const status = forcedStatus ?? (code.includes('MFA') ? HttpStatus.FORBIDDEN : code === 'REQUEST_NOT_OPEN' ? HttpStatus.CONFLICT : HttpStatus.BAD_REQUEST);
...
export class ApprovalController {
  ...
  async list(@Req() request: unknown, @Query('status') status?: string): Promise<unknown> {
    ...
    const statuses = status === undefined ? undefined : status.split(',').filter(Boolean) as any;
    return this.repository.withTransaction(context, (tx) => tx.findRequests(context, statuses ? { statuses } : undefined));
  }

  @Get('requests/:requestId')
  @ApiOperation({ summary: 'Read one tenant-scoped approval request and its decisions' })
  @ApiOkResponse({ schema: { type: 'object', required: ['request', 'decisions'], properties: { request: { type: 'object', additionalProperties: true }, decisions: { type: 'array', items: { type: 'object', additionalProperties: true } } } } })
  ...
  async publishPolicy(@Req() request: unknown, @Body() input: CreateApprovalPolicyDto): Promise<unknown> {
    const context = await this.requestContext.resolve(request);
  ...
  @ApiOkResponse({ schema: { type: 'object', additionalProperties: true } })
  async openRequest(@Req() request: unknown, @Body() input: CreateApprovalRequestDto): Promise<unknown> {
    const context = await this.requestContext.resolve(request);
  ...
  @ApiOkResponse({ schema: { type: 'object', additionalProperties: true } })
  async decide(@Req() request: unknown, @Param('requestId') requestId: string, @Body() input: DecideApprovalDto): Promise<unknown> {
    if (input.mfaAssertionId.length === 0 || !parseStableIdentifierV1(requestId).accepted) fail('INVALID_IDENTIFIER');
    const context = await this.requestContext.resolve(request);
    const actor = this.iam ? await this.iam.findMembership(context, context.actorId) : undefined;
    if (!actor || actor.roleId !== input.actorRole) fail('ACTOR_ROLE_MISMATCH', HttpStatus.FORBIDDEN);
    const parsedId = parseStableIdentifierV1(requestId);
    if (!parsedId.accepted) fail('INVALID_IDENTIFIER');
    const result = await this.approval.decide(context, { ...input, requestId: parsedId.value, actorId: context.actorId });
    if (!result.accepted) fail(result.code);
    return result.value;
  }
}
