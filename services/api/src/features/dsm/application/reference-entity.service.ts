...
export type ReferenceEntityServiceResultV1<TValue> =
  | ReferenceEntityResultV1<TValue>
  | { readonly accepted: false; readonly code: ReferenceEntityServiceErrorV1 };
...
export class ReferenceEntityService {
  ...
  public async create(
    ...
    input: Parameters<typeof createBusinessPartyVersionV1>[0],
  ...
  public async merge(
    ...
    if (!sourceEntityId.accepted || !targetEntityId.accepted || !actorId.accepted)
      return Object.freeze({ accepted: false as const, code: 'INVALID_IDENTIFIER' as const });
    if (actorId.value !== context.actorId)
      return Object.freeze({ accepted: false as const, code: 'ACTOR_MISMATCH' as const });
    return this.repository.withTransaction(context, async (transaction) => {
      ...
      if (!source || !target)
        return Object.freeze({ accepted: false as const, code: 'ENTITY_NOT_FOUND' as const });
      const resolution = mergeBusinessPartyVersionsV1({
        source,
        target,
        ...input,
  ...
  public async listVersions(
    ...
    return this.repository.withTransaction(context, (transaction) =>
      transaction.listVersions(context, entityId),
    );
  }
  ...
  public async listResolutions(
    ...
    return this.repository.withTransaction(context, (transaction) =>
      transaction.listResolutions(context, entityId),
    );
  }
}
