...
export type RuleSetServiceResultV1<TValue> = RuleSetResultV1<TValue> | { readonly accepted: false; readonly code: RuleSetServiceErrorV1 };

export class RuleSetService {
  public constructor(private readonly repository: RuleSetRepositoryPortV1) {}

  public async create(context: IamTenantContextV1, input: Parameters<typeof createRuleSetDefinitionV1>[0]): Promise<RuleSetServiceResultV1<RuleSetDefinitionV1>> {
  ...
  public async publish(context: IamTenantContextV1, versionId: StableIdentifierV1, nextVersionIdInput: unknown, publishedAt: unknown): Promise<RuleSetServiceResultV1<RuleSetDefinitionV1>> {
    return this.repository.withTransaction(context, async (transaction) => {
      const current = await transaction.find(context, versionId);
      if (!current) return Object.freeze({ accepted: false as const, code: 'VERSION_NOT_FOUND' as const });
  ...
  public async list(context: IamTenantContextV1, datasetId: StableIdentifierV1): Promise<readonly RuleSetDefinitionV1[]> {
