...
function clone(definition: RuleSetDefinitionV1): RuleSetDefinitionV1 {
  return Object.freeze({
    ...
    rules: Object.freeze(definition.rules.map((rule) => Object.freeze({ ...rule, parameters: Object.freeze({ ...rule.parameters }) }))),
  });
}
...
export class InMemoryRuleSetRepositoryAdapter implements RuleSetRepositoryPortV1 {
  ...
  public async save(context: IamTenantContextV1, definition: RuleSetDefinitionV1): Promise<void> {
    ...
    if (existing && JSON.stringify(existing) !== JSON.stringify(definition)) throw new Error('DSM_IMMUTABLE_RULE_SET');
    this.definitions.set(definition.versionId, clone(definition));
  }

  public async find(context: IamTenantContextV1, versionId: StableIdentifierV1): Promise<RuleSetDefinitionV1 | undefined> {
    await Promise.resolve();
  ...
  public async withTransaction<TValue>(context: IamTenantContextV1, work: (transaction: RuleSetTransactionPortV1) => Promise<TValue>): Promise<TValue> {
    let release!: () => void;
