...
function cloneVersion(version: BusinessPartyVersionV1): BusinessPartyVersionV1 {
  return Object.freeze({
    ...
    externalIdentifiers: Object.freeze(
      version.externalIdentifiers.map((item) => Object.freeze({ ...item })),
    ),
  });
}
...
export class InMemoryReferenceEntityRepositoryAdapter implements ReferenceEntityRepositoryPortV1 {
  ...
  public async listVersions(
    ...
    return [...this.versions.values()]
      .filter(
        (version) =>
          version.entityId === entityId && visible(context.tenantScope, version.tenantScope),
      )
  ...
  public async saveResolution(
    ...
    await Promise.resolve();
    const source = [...this.versions.values()].find(
      (candidate) => candidate.entityId === resolution.sourceEntityId,
    );
    const target = [...this.versions.values()].find(
      (candidate) => candidate.entityId === resolution.targetEntityId,
    );
    if (
      !source ||
      !target ||
      !visible(context.tenantScope, source.tenantScope) ||
      !visible(context.tenantScope, target.tenantScope)
    )
      throw new Error('DSM_REFERENCE_ENTITY_NOT_FOUND');
    const existing = this.resolutions.get(resolution.resolutionId);
    if (existing && JSON.stringify(existing) !== JSON.stringify(resolution))
      throw new Error('DSM_IMMUTABLE_REFERENCE_RESOLUTION');
  ...
  public async listResolutions(
    ...
    return [...this.resolutions.values()]
      .filter((resolution) => {
        ...
        return (
          (resolution.sourceEntityId === entityId || resolution.targetEntityId === entityId) &&
          source !== undefined &&
          visible(context.tenantScope, source.tenantScope)
        );
      })
