...
function context(idempotencyKey: string) {
  const result = createIamTenantContextV1({ tenantScope: { scopeType: 'workspace', organizationId, workspaceId }, actorId, correlationId, idempotencyKey, authorizationEpoch: 2 });
...
void test('[IAE-005] service issues and resolves an epoch-bound grant', async () => {
  ...
  const resolved = await service.resolve(context('grant-resolve'), { grantId: input.grantId, recipientDeviceId: deviceId, authorizationEpoch: 2, now: '2026-01-01T00:01:00.000Z' });
...
void test('[IAE-005, IAM-020] revoked, expired, and mismatched grants fail closed', async () => {
  ...
  assert.deepEqual(await service.resolve(context('grant-device'), { grantId: input.grantId, recipientDeviceId: '00000000-0000-4000-8000-000000000099', authorizationEpoch: 2, now: '2026-01-01T00:01:00.000Z' }), { accepted: false, code: 'DEVICE_MISMATCH' });
  assert.deepEqual(await service.resolve(context('grant-epoch'), { grantId: input.grantId, recipientDeviceId: deviceId, authorizationEpoch: 3, now: '2026-01-01T00:01:00.000Z' }), { accepted: false, code: 'EPOCH_MISMATCH' });
  assert.deepEqual(await service.resolve(context('grant-expired'), { grantId: input.grantId, recipientDeviceId: deviceId, authorizationEpoch: 2, now: '2026-01-01T00:06:00.000Z' }), { accepted: false, code: 'GRANT_EXPIRED' });
