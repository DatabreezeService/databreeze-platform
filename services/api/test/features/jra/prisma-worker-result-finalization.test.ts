...
const descriptor = (() => {
  const result = createExecutionRequestDescriptorV1({
    ...
    inputObjectIds: sourceArtifactVersionIds,
...
class Database {
  actions: JraWorkerActionDatabaseRowV1[] = [
    {
      id: id('40'),
  ...
  jobs: JraWorkerJobDatabaseRowV1[] = [
    {
      id: id('4'),
      scopeType: 'workspace',
      organizationId: id('1'),
  ...
  attempts: JraWorkerAttemptDatabaseRowV1[] = [
    {
      id: id('5'),
      jobId: id('4'),
      scopeType: 'workspace',
  ...
  descriptors: JraWorkerExecutionRequestDatabaseRowV1[] = [
    {
      id: id('6'),
      jobId: id('4'),
      stepId: id('20'),
  ...
  constructor() {
    this.client = {
      typedActionDefinitionRecord: delegate(
        () => this.actions,
        (v) => {
          this.actions = v;
        },
      ...
      executionAttemptRecord: delegate(
        () => this.attempts,
        (v) => {
          this.attempts = v;
        },
      ),
      executionRequestDescriptorRecord: delegate(
        () => this.descriptors,
        (v) => {
          this.descriptors = v;
        },
      ),
      workerCompletionRecord: delegate(
        () => this.completions,
        (v) => {
          this.completions = v;
        },
      ),
      workerResultPreparationRecord: delegate(
        () => this.preparations,
        (v) => {
          this.preparations = v;
        },
      ),
      workerResultFinalizationRecord: delegate(
        () => this.finalizations,
        (v) => {
          this.finalizations = v;
        },
      ),
      resultManifestRecord: delegate(
        () => this.manifests,
        (v) => {
          this.manifests = v;
        },
      ),
      jobTransitionRecord: delegate(
        () => this.transitions,
        (v) => {
          this.transitions = v;
        },
      ),
      jobOutboxRecord: delegate(
        () => this.outbox,
        (v) => {
          this.outbox = v;
        },
  ...
  effectsPort(): WorkerResultFinalizationEffectsPortV1 {
    return {
      commit: async (_transaction, effect) => {
...
function adapter(database: Database) {
  return new PrismaJraWorkerAdapter(
    database.client,
    { isCurrent: async () => true },
    {
      ...
      acceptResultReferences: async () => {
        throw new Error('legacy must be unused');
      },
    },
...
async function prepared(database: Database) {
  ...
  if (!result.accepted) throw new Error('prepare failed');
  return result.preparation;
}
function finalizeInput(
  preparation: Awaited<ReturnType<typeof prepared>>,
    ...
    attestations: [
      {
        ...
        jobId: id('4'),
        attemptId: id('5'),
        executionDescriptorId: id('6'),
        executionDescriptorHash: descriptor.canonicalHash,
        submissionId: preparation.submissionId,
        artifactVersionId: id('51'),
        contentSha256: 'f'.repeat(64),
        contentLength: 512,
        mediaType: 'application/json',
        sourceLineageHash,
...
void test('[JRA-007/JRA-031] preparation is stable and leaves attempt/job non-terminal', async () => {
  ...
  const first = await prepared(database);
  const second = await prepared(database);
  assert.equal(first.submissionId, second.submissionId);
  assert.equal(database.preparations.length, 1);
  ...
  assert.deepEqual(first.subjectBindings, {
    dashboardId: id('29'),
    dashboardVersionId: id('30'),
    datasetVersionId: id('34'),
...
void test('[JRA-012/JRA-023/JRA-031] finalization commits manifest, terminal state, replay, outbox and participants once', async () => {
  const database = new Database();
  const input = finalizeInput(await prepared(database));
  const service = adapter(database);
  ...
  assert.equal(database.finalizations.length, 1);
  assert.equal(database.attempts[0]?.state, 'SUCCEEDED');
  assert.equal(database.jobs[0]?.state, 'SUCCEEDED');
  assert.equal(database.outbox.length, 1);
  assert.equal(database.effects, 1);
...
void test('[JRA-007/JRA-031] older attempt preparation is rejected after a newer attempt exists', async () => {
  const database = new Database();
  database.attempts.push({ ...database.attempts[0]!, id: id('60'), attemptNumber: 2 });
  const result = await adapter(database).prepare({
    identity,
    authorization: authorization(),
    leaseTokenHash,
