...
const attempt = {
  ...
  leaseTokenHash,
  leaseExpiresAt: utc('2026-08-13T00:10:00.000Z'),
  ...
  const attestations: WorkerResultAttestationResolverPortV1 = {
    resolveAttestation: async ({ attestationId }) => {
      ...
      return {
        ...
        submissionId: id('7'),
        executionDescriptorHash: descriptorHash,
        outputPolicyHash: '3'.repeat(64),
        artifactVersionId: id('11'),
        contentSha256: 'f'.repeat(64),
        contentLength: 512,
        mediaType: 'application/json',
        sourceLineageHash: '1'.repeat(64),
        finalizedAt: utc(now),
      };
    },
  };
  const finalization: WorkerResultFinalizationPortV1 = {
    ...
    finalize: async (input) => {
      ...
      return {
        ...
        completion: {
          submissionId: id('7'),
          resultManifestId: id('12'),
          resultManifestHash: '2'.repeat(64),
          attemptId: attempt.attemptId,
          jobId: job.jobId,
          outcome: 'SUCCEEDED',
          attemptRevision: 2,
          jobRevision: 3,
          artifactVersionIds: [id('11')],
        },
      };
    },
  };
  const boundary = new WorkerBoundary({
    authenticator: { authenticate: async () => identity },
    authority,
    attempts: {} as never,
    grants: {} as never,
    completion: {} as never,
