...
function context(workspaceIdValue: string, idempotencyKey: string) {
  const result = createIamTenantContextV1({
    tenantScope: { scopeType: 'workspace', organizationId, workspaceId: workspaceIdValue },
    actorId,
    correlationId,
    idempotencyKey,
...
void test('[DSM-007, DSM-008] mapping service versions and publishes definitions', async () => {
  ...
  const published = await service.publish(
    context(workspaceId, 'mapping-publish'),
  ...
  assert.equal(
    (await service.list(context(workspaceId, 'mapping-list'), stable(input.datasetId))).length,
    2,
  );
});
...
void test('[IAM-009, DSM-007] sibling workspaces cannot read mappings', async () => {
  ...
  assert.equal(
    (await service.list(context(siblingWorkspaceId, 'mapping-scope-list'), stable(input.datasetId)))
      .length,
    0,
  );
});
