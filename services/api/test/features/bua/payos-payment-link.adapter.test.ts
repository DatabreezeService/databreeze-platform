...
void test('[BUA-001, BUA-002] production PayOS adapter sends only the server catalog amount', async () => {
  ...
  const adapter = new PayosPaymentLinkAdapter({
    ...
    fetch: async (_input, init) => {
      ...
      return new Response(JSON.stringify({ code: '00', data: { checkoutUrl: 'https://payos.example/checkout/1' } }), { status: 200, headers: { 'content-type': 'application/json' } });
    },
  });
...
void test('[BUA-007] production PayOS webhook verifies canonical signature and rejects tampering', () => {
  ...
  const data = { orderCode: 123456, amount: 149_000, code: '00', reference: 'provider-event-1', currency: 'VND' };
  const canonical = Object.keys(data).sort().map((key) => `${key}=${data[key as keyof typeof data]}`).join('&');
  const signature = createHmac('sha256', checksumKey).update(canonical).digest('hex');
  const adapter = new PayosPaymentLinkAdapter({
    clientId: 'client', apiKey: 'api', checksumKey,
    successUrl: 'https://web.example/success', failedUrl: 'https://web.example/failed',
  });
