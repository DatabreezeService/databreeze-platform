...
export function createPkceChallengeV1(
  ...
  if (!validVerifier(verifierInput)) return Object.freeze({ accepted: false, code: 'INVALID_VERIFIER' });
  try {
...
export function verifyPkceChallengeV1(
  ...
  if (typeof challengeInput !== 'string' || !/^[A-Za-z0-9_-]{43}$/u.test(challengeInput)) return false;
...
export function isAllowedRedirectUriV1(input: unknown): boolean {
  ...
  if (input === 'com.databreeze.desktop:/oauth2redirect' || input === 'com.databreeze.android:/oauth2redirect')
    return true;
