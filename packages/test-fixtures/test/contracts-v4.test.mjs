...
const schemaManifest = JSON.parse(readFileSync(resolve(fixtureRoot, fixtureManifest.schemaManifest), 'utf8'));
...
test('v4 auth fixtures cover every v4 schema with accepted and rejected payloads', () => {
  ...
  const schemaIds = new Set(schemaManifest.schemas.filter((entry) => entry.id.includes('/contracts/v4/')).map((entry) => entry.id));
  ...
  for (const entry of schemaManifest.schemas) {
    ajv.addSchema(JSON.parse(readFileSync(resolve(packageRoot, '../contracts', entry.path), 'utf8')));
  }
  for (const schemaId of schemaIds) {
    const cases = fixtureManifest.cases.filter((fixtureCase) => fixtureCase.schemaId === schemaId);
    assert.deepEqual([...new Set(cases.map((fixtureCase) => fixtureCase.expectedAcceptance))].sort(), [false, true]);
    ...
    for (const fixtureCase of cases) {
      ...
      assert.equal(validate(JSON.parse(readFileSync(sourcePath, 'utf8'))), fixtureCase.expectedAcceptance, `${fixtureCase.id}: ${ajv.errorsText(validate.errors)}`);
    }
  }
});
